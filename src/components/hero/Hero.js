...
const Hero = ({ title, subtitle, media, mobileVideo, mp4Fallback, video, poster }) => {
    ...
    React.useEffect(() => {
        ...
        function handleClick() {
            ...
            if (
                ...
                const mobileSource = !supportsWebm && mp4Fallback ? mp4Fallback : mobileVideo;
                const desktopSource = !supportsWebm && mp4Fallback ? mp4Fallback : media;
    ...
    return (
        <section className="relative h-[420px] md:h-screen-minus-100 sm-full-bleed">
            <div className="absolute z-10 text-white bottom-8 left-8 lg:bottom-16 lg:left-16">
                <p className="text-base md:text-3xl">{subtitle}</p>
                <p className="text-3.5xl md:text-7xl">{title}</p>
            </div>
            {video && (
                <>
                    <video
